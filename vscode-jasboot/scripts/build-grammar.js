...
const groups = JSON.parse(
  fs.readFileSync(path.join(__dirname, "keywords-groups.json"), "utf8"),
);
...
function supportPatternList() {
  const chunks =
    groups.supportChunks && groups.supportChunks.length
      ? groups.supportChunks
      : [""];
  return chunks.map((chunk, i) => {
    const key = `keywords_support_${i + 1}`;
    if (!chunk)
      return { key, rule: pat("(?!)", "support.function.builtin.jasboot") };
    return {
      key,
      rule: pat(`\\b(${chunk})\\b`, "support.function.builtin.jasboot"),
    };
  });
}
...
const grammar = {
  ...
  repository: {
    ...
    function_declaration: {
      match: "\\b(funcion)\\s+([a-zA-Z_][a-zA-Z0-9_]*)",
      captures: {
        1: { name: "keyword.control.jasboot" },
        2: { name: "entity.name.function.jasboot" },
      },
    },

    function_call: {
      match: "\\b([a-zA-Z_][a-zA-Z0-9_]*)\\s*(?=\\()",
      name: "entity.name.function.jasboot",
    },
    ...
    class_declaration: {
      ...
      captures: {
        1: { name: "keyword.control.jasboot" },
        2: { name: "entity.name.type.class.jasboot" },
      },
    },

    struct_declaration: {
      match: "\\b(registro)\\s+([a-zA-Z_][a-zA-Z0-9_]*)",
      captures: {
        1: { name: "keyword.control.jasboot" },
        2: { name: "entity.name.type.struct.jasboot" },
      },
    },
    ...
    interpolated_expr: {
      patterns: [
        beginEnd("\\{", "\\}", "meta.block.interpolation.jasboot", [
          { include: "#interpolated_expr" },
        ]),
    ...
    strings_concept: beginEnd(
      "'",
      "'",
      "string.quoted.single.concept.jasboot",
    ...
    numbers: {
      patterns: [
        {
          name: "constant.numeric.hex.jasboot",
    ...
    operators: {
      patterns: [
        ...
        {
          name: "keyword.operator.comparison.jasboot",
          match: "(===|==|!=|<=|>=|\\+\\+|--|\\+=|-=|\\*=|/=|<<|>>)",
        },
        ...
        {
          name: "punctuation.definition.bracket.jasboot",
          match: "[()\\[\\]{}]",
        },
        { name: "punctuation.separator.comma.jasboot", match: "," },
    ...
    keywords_control: pat(
      `\\b(${groups.control})\\b`,
      "keyword.control.jasboot",
    ),
    keywords_declarative: pat(
      `\\b(${groups.declarative || "app|vista|componente|tema|rutas|columna|fila|tarjeta|texto|titulo|subtitulo|boton_ruta|boton_alerta|boton_secundario|jasb"})\\b`,
      "keyword.declarative.jasboot",
    ),
    keywords_storage: pat(`\\b(${groups.storage})\\b`, "storage.type.jasboot"),
    keywords_word_op: pat(
      `\\b(${groups.wordOp})\\b`,
      "keyword.operator.word.jasboot",
    ),
    keywords_import: pat(
      `\\b(${groups.imports})\\b`,
      "keyword.control.import.jasboot",
    ),
    keywords_constant: pat(
      "\\b(verdadero|falso)\\b",
      "constant.language.jasboot",
    ),
    keywords_io: pat(
      `\\b(${groups.io})\\b`,
      "support.function.builtin.io.jasboot",
    ),

    forbidden_english: pat(
      `\\b(${forbidden})\\b`,
      "invalid.illegal.name.jasboot",
    ),
      ...
      patterns: [
        {
          ...
          name: "entity.name.type.jasboot",
        },
        {
          match: "\\b[a-zA-Z_][a-zA-Z0-9_]*\\b",
          name: "variable.other.readwrite.jasboot",
        },
      ],
    },
  },
